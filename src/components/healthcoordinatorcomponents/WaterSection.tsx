...
import { Droplets, Camera, UploadCloud, Wifi } from "lucide-react";
...
const WaterSection: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"iot" | "onspot" | "reports">("iot");
  ...
  const recordFromIoT = () => {
    ...
    // copy subset of params to an on-spot submission
    ...
    <div className="space-y-6 animate-fade-in">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <nav className="flex rounded-md bg-muted p-1">
                <button
                  className={`px-3 py-1 text-sm rounded ${activeTab === "iot" ? "bg-background shadow" : "text-text-secondary"}`}
                  onClick={() => setActiveTab("iot")}
                >
                  IoT
                </button>
                <button
                  className={`px-3 py-1 text-sm rounded ${activeTab === "onspot" ? "bg-background shadow" : "text-text-secondary"}`}
                  onClick={() => setActiveTab("onspot")}
                >
                  On-spot
                </button>
                <button
                  className={`px-3 py-1 text-sm rounded ${activeTab === "reports" ? "bg-background shadow" : "text-text-secondary"}`}
        ...
        <CardContent>
          ...
          {activeTab === "onspot" && (
            <form onSubmit={(e) => { e.preventDefault(); submitOnSpot(e); }} className="space-y-4">
