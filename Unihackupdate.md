...
```
                        ...
                        │    Brand fields, Part_Manuf)  │
                        └───────────────┬───────────────┘
                                        ▼
        ┌───────────────────────────────────────────────────┐
        │  LAYER 1 — ENRICHMENT CORE                          │
        │                                                     │
        │  Step 1  Input Cleaning                             │
        │          → strip placeholders ("--Unbranded--" etc) │
        │                                                     │
        │  Step 2  Brand/Manufacturer Normalization            │
        │          → fuzzy-match raw string to the             │
        │            27,000-row Manufacturer & Brand List       │
        │                                                     │
        │  Step 3  Taxonomy Classification                     │
        │          → assign Dept > Class > Fine (classpath)    │
        │            using description text + LOV structure     │
        │                                                     │
        │  Step 4  Attribute Extraction                        │
        │          → pull size, material, voltage, etc. from    │
        │            free text                                 │
        │                                                     │
        │  Step 5  Attribute Normalization                     │
        │          → snap each value to the nearest allowed     │
        │            LOV value (never invent new values)        │
        │          → convert units to approved UOM abbreviation │
        │          → convert decimal ↔ fraction where needed    │
        │                                                     │
        │  Step 6  Enrichment from Manufacturer Sources         │
        │          → fill gaps by retrieving from the           │
        │            manufacturer's own site/docs only          │
        │            (never marketplaces/distributors)          │
        │                                                     │
        │  Step 7  Description Generation                      │
        │          → build Invoice / Mobile / Title / Short /   │
        │            Long descriptions per the content-           │
        │            guideline formulas & char limits            │
        │                                                     │
        │  Step 8  Confidence Scoring                           │
        │          → flag low-confidence fields as               │
        │            "needs human review" instead of guessing    │
        └───────────────────────────┬───────────────────────┘
                                    ▼
                    ┌───────────────────────────────┐
                    │   STRUCTURED PRODUCT RECORD     │
                    │   (matches the 252-column        │
                    │    Delivery Format schema)        │
                    └───────────────┬───────────────┘
                                    ▼
        ┌───────────────────────────────────────────────────┐
        │  LAYER 2 — ASSURANCE & DISCOVERY                    │
        │                                                     │
        │  Product Assurance Engine (PAE)                     │
        │    → periodically re-checks each enriched record      │
        │      against its manufacturer source                  │
        │    → flags drift (spec changed, discontinued, etc.)   │
        │    → routes flagged items to Alert Center for a        │
        │      human to Accept & Continue / Search Alternative /│
        │      Dismiss                                          │
        │                                                     │
        │  TurboVec Semantic Search                            │
        │    → vector index built ON TOP of Layer 1 output       │
        │    → lets buyers/agents find products by meaning,      │
